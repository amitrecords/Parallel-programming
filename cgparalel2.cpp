...
double* Conjugate_gradient(int start , int en, int n, int rowptac[] ,int colac[] ,double valac[] ,double b[])
   {
	  const int size=n;
	  double r[size],res[size],d[size],z[size],minv[size][size];
...
{ 
    ...
    kmax=MAX-2;
    int i,j;
    double a[nsquare][nsquare]={ 0.0 },AT[nsquare][nsquare]= { 0 };
  ...
  // Assigning values to the matrix B (Ax=B)
     ...
     //Ending at the topmost inner row
       	...
       	MPI_Init(&argc,&argv);
       	 ...
       	 // ROW BLOCKING-*******
       	         int start=rn*((nsquare)/NP);
       	         int en=(int)(rn+1) * ((nsquare)/NP);
       	            if(rn==(NP-1))
       	          en=en+(MAX % NP);
       	         //**********************

  // Conjugate gradient solver
	 Conjugate_gradient(start,en,nsquare,rowptac,colac,valac,b);
     ...
     for(pnum=0;pnum<NP;pnum++)
 	  {if(rn==pnum)
	     {//Displaying the matrix u
	  	      //	cout<<" The matrix u:"<< start<<" " << en<<endl;
	  	       	for(i=(start/(MAX-2));i<(en/(MAX-2))+1;i++)
	  	        	{  if(rn!=0 && i==(start/(MAX-2)))
 	           	          	 {i=i+1;}

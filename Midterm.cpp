...
//Displaying the coefficient matrix	
  ...
  // Assigning values to the matrix B (Ax=B)
     //starting from the bottom inner row
    	{  for(i=1;i<MAX-2;i++)
    	   b[0][i]= -ut[0][i];
    	   b[0][0]= -ut[1][0]-ut[0][1]   ;
           b[0][MAX-3]= -ut[1][MAX-1] - ut[0][MAX-2]; 	
        }  
     //the rows further upwards
    	 ...
    	 for(j=2;j<=MAX-3;j++) 
    	{
          ...
          b[0][j*(MAX-2)-1]= -u[j][MAX-1]; 	     
        }  
     //Ending at the topmost inner row
       	{  
        	   b[0][(MAX-2)*(MAX-3)]= -u[MAX-2][0];
         	   b[0][(MAX-2)*(MAX-2)-1]= -u[MAX-2][MAX-1];
 ...
 //Displaying the matrix B	
    	...
    	{  
	      ...
	      cout<<"size of b:"<<sizeof(b)/sizeof(b[0][0])<<endl;  

// Solve the linear system
	         dgesv_(&n, &nrhs, &AT[0][0], &lda, ipiv, &b[0][0], &ldb, &info);        
